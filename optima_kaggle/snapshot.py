...
This module owns the single point where base.json is downloaded: once per
run, hashed, snapshotted immutably under
``runs/base-<sha12>/base/base.json``, and validated. Everything downstream
(enrichment, embedding, evaluation) reads that snapshot path and never
re-downloads or regenerates it.
"""
...
class RunContext:
    ...
    def create(
        ...
        manifest.setdefault("base_url", base.url)
